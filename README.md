...
This provisioner includes a built in NFS server, and is not intended for
connecting to a pre-existing NFS server. If you have a pre-existing NFS Server,
please consider using the [NFS Client Provisioner
...
This chart bootstraps a [nfs-server-provisioner
...
```console
$ helm install my-release stable/nfs-server-provisioner
```
...
Configurable parameters and their default values can be found in [the
...
Longhorn recommended to setup a liveness check on the workloads to makes sure
the pods would get restarted on NFS server failures. E.g.

```
timeout 10 ls /mnt/nfs-mountpoint.
```

```json
"livenessProbe": {
  "exec": {
    "command": ["bash", "-c", "timeout 10 ls /mnt/nfs-mountpoint"]
  }
}
```

The timeout command is part of the liveness check to work around [this
Kubernetes issue](https://github.com/kubernetes/kubernetes/issues/26895).
